...
- [前提介绍](#前提介绍)
- [下载镜像包和helm包](#下载镜像包和helm包)
- [一键式部署](#一键式部署)
- [安装命令](#安装命令)
- [验证monitor](#验证monitor)
- [验证station](#验证station)
- [配置k8s config文件路径](#配置k8s-config文件路径)
...
```bash
wget https://nexus.corp.dy-sec.com/repository/dysec-general-anonymous/minhang/product-1.3.tgz
tar -zxvf product-1.3.tgz
# 进入到工作目录
cd product
```
物料包解压后目录结构如下所示：
...
### 验证monitor
- 向wparse发送日志数据，验证monitor是否能正确接收和展示日志数据。
```bash
printf '180.57.30.148 - - [21/Jan/2025:01:40:02 +0800] "GET /nginx-logo.png HTTP/1.1" 500 368 "<http://207.131.38.110/>" "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.142 Safari/537.36" "-"\n' | nc -u 127.0.0.1 31600
```
